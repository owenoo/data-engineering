  ...
  File.open(@filepath + @fileinfo.original_filename) do |fi|
    begin 
      ...
      # the file one line at a time after it has been uploaded to the server	
            ...
            #p = Purchaser.where(name:  myHash['purchaserName'])
            #if(p.empty?)            
              #new_purchaser = Purchaser.new(:name => myHash['purchaserName']) 
              #new_purchaser.save
              #p = Purchaser.where(name:  myHash['purchaserName'])
            #end
            p = Purchaser.get_set(myHash['purchaserName'])                          
            
            #m = Merchant.where(name: myHash['merchName'])
            #if(m.empty?)
              #new_merchant = Merchant.new(:name => myHash['merchName'], :address => myHash['merchAddress'])
              #new_merchant.save
              #m = Merchant.where(name: myHash['merchName'])
            #end
            m = Merchant.get_set(myHash['merchName'], myHash['merchAddress'])

            #i = Item.where(description: myHash['itemDesc'])
            #if(i.empty?)
              #new_item = Item.new(:description => myHash['itemDesc'], :price => myHash['itemPrice'])
              #new_item.save  
              #i = Item.where(description: myHash['itemDesc'])
            #end 
